...
diputadosUrl = baseUrl + "/portal/page/portal/Congreso/Congreso/Diputados/BajasLegAct?_piref73_2496066_73_2496053_2496053.next_page=/wc/diputadosBajaLegActual&paginaActual=%d"
...
begin 
  ...
  diputados = dataDiputados.css('.listado_1 ul li')
  diputados.each do |diputado|
    logger.info diputado.to_s
    # Inicialización
    legislaturaId = 1 # Hardcoded id for the legislature  X
    diputadoUrl = diputado.at_css('a').attr('href')
    diputadoNombre = diputado.at_css('a').text.slice(/^([^\(]*)\(.*\)$/i, 1).strip
    diputadoGP = diputado.at_css('a').text.slice(/^[^\(]*\((.*)\)$/i, 1).downcase!
